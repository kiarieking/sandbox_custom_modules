...
class Dispatch(models.Model):
    ...
    def action_cancel(self):
        ...
        order_nos = [line.order_no for line in self.order_ids]
    ...
    def _action_post_quotation_lines(self):
        ...
        order_nos = [sale_order_line_id.order_no for sale_order_line_id in sale_order_line_ids]
        
        lines = [(0, 0, {
            "date": line.order_id.date_dispatch,
            "order_no": line.order_no,
            "product_id": line.product_id.id,
            'product_uom' : line.product_id.uom_id.id,
            "name": (line.order_id.vehicle_id.license_plate + ":" + line.description) or line.order_id.vehicle_id.license_plate,
            "notes": line.notes,
    ...
    def _action_post_purchase_lines(self):
        ...
        for line in self.order_ids:                
            vals = {
                ...
                "vendor_id": line.order_id.vendor_id.id,
                "reference": line.order_id.name,
                "description": (line.order_id.vehicle_id.license_plate + ":" + line.description) or line.order_id.vehicle_id.license_plate,
                "quantity": line.quantity,
                # "carrier_price": order_id.product_id.standard_price,
                "carrier_price": line.carrier_price
            }
    ...
    def find_orphan_fuel_vouchers(self):
        """
        Finds and links unreferenced fuel vouchers to the current dispatch document.

        This method searches for fuel vouchers issued for the same vehicle on
        the dispatch date or the previous day. It then links the first unreferenced
        voucher found to this dispatch document using its name as the reference number.
        """
        # Ensure we have the necessary data to proceed
        if not self.date_dispatch or not self.vehicle_id or not self.name:
            return

        previous_day = self.date_dispatch - timedelta(days=1)

        fuel_vouchers = self.env['fleet.vehicle.log.services'].search([
            ('date', '>=', previous_day),
            ('date', '<=', self.date_dispatch),
            ('vehicle_id', '=', self.vehicle_id.id),
            ('description', '=', None),
        ])
        
        # Pre-check if the current dispatch name is already used as a reference
        # This avoids iterating if the link already exists through another voucher
        if any(voucher.description == self.name for voucher in fuel_vouchers):
            return

        # Find and link the first unreferenced voucher
        for voucher in fuel_vouchers:
            if not voucher.description:
                voucher.write({'description': self.name})
                return
